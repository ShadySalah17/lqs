...
class WC_Name_Your_Price_Display {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_nyp_after_price_input', array( $this, 'display_hidden_update_input' ), 20 );
		...
		add_action( 'woocommerce_after_variations_form', array( $this, 'revert_display_after_variable_product' ) );
	...
	public function display_hidden_update_input(){
		if ( isset( $_GET[ 'update-price' ] ) ) {
			$updating_cart_key = wc_clean( $_GET[ 'update-price' ] );
			if ( isset( WC()->cart->cart_contents[ $updating_cart_key ] ) ) {
				echo '<input type="hidden" name="update-price" value="' . $updating_cart_key . '" />';
			}
		}
	}
	...
	public function single_add_to_cart_text( $text, $product ) {
		...
		if( WC_Name_Your_Price_Helpers::is_nyp( $product ) ){
			$nyp_text = apply_filters( 'wc_nyp_single_add_to_cart_text', get_option( 'woocommerce_nyp_button_text_single', '' ), $product );
	...
	public function add_to_cart_text( $text, $product ) {
		...
		if ( WC_Name_Your_Price_Helpers::is_nyp( $product ) ){
			$text = apply_filters( 'wc_nyp_add_to_cart_text', get_option( 'woocommerce_nyp_button_text', __( 'Set Price', 'wc_name_your_price' ) ), $product );
		}
	...
	public function add_edit_link_in_cart( $content, $cart_item, $cart_item_key ) { 
		...
		if( isset( $cart_item['nyp'] ) && ! apply_filters( 'wc_nyp_isset_disable_edit_it_cart', isset( $cart_item['subscription_resubscribe'] ), $cart_item, $cart_item_key ) ) {
			...
			if ( function_exists( 'is_cart' ) && is_cart() && ! $this->is_cart_widget() ) {
				...
				$nyp_prefix = WC_Name_Your_Price_Helpers::get_prefix( $nyp_id );

				$edit_in_cart_link = add_query_arg( array(
					'update-price' => $cart_item_key,
					'nyp' . $nyp_prefix => $cart_item['nyp'],
				), $_product->get_permalink() );

				if( isset( $cart_item['nyp_period'] ) ){
					$edit_in_cart_link = add_query_arg( 'nyp-period' . $nyp_prefix, $cart_item['nyp_period'], $edit_in_cart_link );
				}
