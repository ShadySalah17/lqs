...
class WC_Name_Your_Price_Helpers {
	...
	public static function get_price_input_label_text( $product ) {
		...
		// If not nyp quit early.
		if ( ! self::is_nyp( $product ) && ! self::has_nyp( $product ) ) {
			return $html;
		}
		...
		$html = sprintf(
			...
			_x( '%1$s ( %2$s )', 'Name your price input label', 'wc_name_your_price' ),
				get_option( 'woocommerce_nyp_label_text', __( 'Name your price', 'wc_name_your_price' ) ),
				get_woocommerce_currency_symbol()
		);

		return esc_html( apply_filters( 'woocommerce_nyp_price_input_label_text', $html, $product ) );
	...
	public static function get_price_input( $product, $prefix = false ) {
		 ...
		 * @param  string $prefix - needed for grouped, composites, bundles, etc.
		 * @return string
		 * @since  2.11.0
		 */
		$attributes = apply_filters( 'wc_nyp_price_input_attributes', $attributes, $product, $prefix );

		$attributes_string = '';

		foreach( $attributes as $key => $attribute ) {
			$attribute = is_array( $attribute ) ? implode ( ' ', $attribute ) : $attribute;
			$attributes_string .= sprintf( '%s="%s" ', esc_attr( $key ), esc_attr( $attribute ) );
		}

		$input = sprintf( '<input %s/>', $attributes_string );

		return apply_filters( 'woocommerce_get_price_input', $input, $product->get_id(), $prefix );
	...
	public static function get_data_attributes( $product, $prefix  = null ) {
		 ...
		 * @since  2.11.0
		 */
		$attributes = apply_filters( 'wc_nyp_data_attributes', $attributes, $product, $prefix );
	 ...
	 *
	 * @param string $prefix - The string to differentiate the NYP price input name, default is null.
	 * @param int $nyp_id - A product|variation ID.
	 * @return string
	 * @since 2.11.0
	 */
	public static function get_prefix( $nyp_id ) {
		return apply_filters( 'nyp_field_prefix', '', $nyp_id );
	}
