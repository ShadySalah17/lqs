...
$ap_settings[ 'post_category' ] = (!empty($post_category))?$post_category:array();
...
if ( isset($_POST[ 'ap_form_id' ]) ) {
    $ap_form_id = intval($_POST['ap_form_id']);
    $wpdb->update(
            $table_name, array(
        'form_details' => $ap_settings_serialized
            ), array( 'ap_form_id' => $ap_form_id ), array(
        '%s'
            ), array( '%d' )
    );
    //$_SESSION[ 'ap_message' ] = __('Form Updated Successfully.', 'anonymous-post-pro');
    wp_redirect(admin_url() . 'admin.php?page=anonymous-post-pro&action=edit_form&form_id=' . $ap_form_id.'&message=1');
} else {
    $wpdb->insert(
            $table_name, array(
        'form_details' => $ap_settings_serialized
    ...
    wp_redirect(admin_url() . 'admin.php?page=anonymous-post-pro&message=1');
}
