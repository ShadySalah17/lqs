...
function qcld_wp_chatbot_load_footer_html(){
    ...
    if ((get_option('disable_wp_chatbot') != 1 && qcld_wp_chatbot_load_controlling() == true) || trim($shortcodetemplate)!='') {
      ...
      <?php echo esc_html__('X', 'wpchatbot'); ?>
                    ...
                    <?php $navnotifications = qcld_wb_chatbot_func_str_replace(unserialize(get_option('wpbot_notification_navigations'))); ?>
		...
		<?php if(get_option('enable_wp_chatbot_sound_initial')==1): ?>
...
function qcld_wb_chatbot_keyword(){
	...
	if(get_option('wp_chatbot_exclude_stock_out_product') == 1){
		$sql = "SELECT p.* FROM ". $wpdb->prefix."posts p join ". $wpdb->prefix."postmeta pm on p.ID = pm.post_id where p.post_type in ('product') and p.post_status='publish' and ((p.post_title REGEXP '[[:<:]]".$searchkeyword."[[:>:]]')) and (pm.meta_key='_stock' and pm.meta_value > '1') group by p.ID";
	}
...
function qcld_wp_chatbot_product_controlling($product_id){
    ...
    if ($product->manage_stock == 'yes' && $product->stock_quantity <= 0 && !$product->backorders_allowed() && get_option('wp_chatbot_exclude_stock_out_product') == 1) {
...
function qcld_wb_chatbot_support_phone(){
    ...
    if(empty($matches)){
        ...
        $subject = (get_option('qlcd_wp_chatbot_callback_email_sub')?get_option('qlcd_wp_chatbot_callback_email_sub'):'WPBot Support Mail Request for Callback');
        //Extract Domain
        $url = get_site_url();
        $url = parse_url($url);
        $domain = $url['host'];
...
<div class="wrap swpm-admin-menu-wrap">
        ...
        <div class="qcld-wpbot-help-section">
            ...
            <div class="qcld-wpbot-section-block">
                ...
                <p>Press <strong>ctrl+b</strong> to open and close chatbot window in frontend.</p>
            </div>
            ...
            <div class="qcld-wpbot-section-block">
                <h2>Keyboard Shortcut</h2>
                <p><strong style="font-weight:bold;">1.</strong> Press <strong>ctrl+b</strong> to open chatbot in frontend.</p>
            </div>
